    ...
    def align_from_url(self, url, person_choose, object_choose):
        ...
        if object_aligned:
            for key,value in object_aligned.items():
                ...
                palavras= [palavra, palavra.title(), palavra.upper(), palavra.lower()]
                for p in palavras:
...
# align.align_from_url(
#     "http://www.bbc.com/news/blogs-trending-40813502",
#     2, 1)


# align = AlignTool()
# URLS = utils.read_lines("/data/alinhador/crawled_objects.txt")  #le o arquivo e coloca em um list de urls
# count = 0
# for url in URLS:  # varre a lista de sites
#     count = count + 1  # aumenta o contador de noticias
    
#     print("Progresso " + str(count) + "/" + str(len(URLS))) 
#     align.align_from_url(url, 1, 1)
